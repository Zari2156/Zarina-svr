...
function upsertAmoLeads(leads) {
  const stmt = db.prepare(`
    INSERT INTO amo_leads (id, name, price, status_id, created_at, closed_at, campaign_id, adset_id, ad_id,
      klass, segment, department, tags, is_qualified, is_success, is_full_payment, synced_at)
...
function upsertGeneralSales(rows) {
  const stmt = db.prepare(`
    INSERT INTO general_sales (id, name, amount, klass, segment, payment_type, is_new, date, manager, synced_at)
    VALUES (@id, @name, @amount, @klass, @segment, @payment_type, @is_new, @date, @manager, @synced_at)
    ON CONFLICT(id) DO UPDATE SET
      name=excluded.name, amount=excluded.amount, klass=excluded.klass, segment=excluded.segment,
      payment_type=excluded.payment_type, is_new=excluded.is_new, date=excluded.date,
